...
where $\bar{R}$ is the average number of runs scored (or allowed) by the league. Dayaratna and Miller showed that Jones and Tappin's linear model is a first-order approximation of James' Pythagorean model.
...
### Current Status
Comparing Jones and Tappin's linear model with James' Pythagorean model (with $\gamma$ = 1.78).